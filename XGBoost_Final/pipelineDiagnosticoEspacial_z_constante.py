...
print("  PIPELINE DIAGNOSTICO: XGBOOST Z CONSTANTE ")
...
# 2 CARGA DEL MODELO CAMPEÓN XGBOOST
print("CARGANDO MODELO XGBOOST PRE-ENTRENADO Z CONSTANTE")
mejor_xgb = joblib.load('modelo_xgboost_z_constante_campeon.joblib')
scaler = joblib.load('escalador_xgboost_z_constante_campeon.joblib')
...
df_metricas.to_csv("metricas_xgb_z_constante.csv", index=False)
print("archivo 'metricas_xgb_z_constante.csv' generado\n")
...
plt.title('Rendimiento Global: XGBoost Real vs. Predicho (Z Constante)', fontsize=14, fontweight='bold')
...
plt.legend()
plt.tight_layout()
plt.savefig('diagnostico_xgb_const_1_real_vs_predicho.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.title('Residual Plot: XGBoost Z Constante', fontsize=14, fontweight='bold')
plt.xlabel('Ganancia Predicha (Log10)', fontsize=12)
plt.ylabel('Residuo (Real - Predicho)', fontsize=12)
plt.tight_layout()
plt.savefig('diagnostico_xgb_const_2_residual_plot.png', dpi=300, bbox_inches='tight')
plt.close()
...
# GRAFICO 3: MAPA ESPACIAL CON SIGNOS (COOLWARM)
...
plt.title('Mapa Espacial del Error: XGBoost Z Constante', fontsize=14, fontweight='bold')
plt.xlabel('Eje X: Longitud del Túnel (metros)', fontsize=12)
plt.ylabel('Eje Y: Ancho (metros)', fontsize=12)
plt.legend(loc='upper right')
plt.grid(True, linestyle=':', alpha=0.6) 
plt.tight_layout()
plt.savefig('diagnostico_xgb_const_3_mapa_espacial.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig('diagnostico_xgb_const_4_cdf.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig('diagnostico_xgb_const_5_qq_plot.png', dpi=300, bbox_inches='tight')
plt.close()
