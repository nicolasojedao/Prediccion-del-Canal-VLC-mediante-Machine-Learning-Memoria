...
print("PIPELINE DE DIAGNOSTICO (Z VARIABLE)")
...
# 2 CARGA DEL MEJOR MODELO Z VARIABLE

print("cargando modelo pre-entrenado z variable")

mejor_mlp = joblib.load('modelo_mlp_z_variable_campeon.joblib')
scaler = joblib.load('escalador_mlp_z_variable_campeon.joblib')
...
df_metricas.to_csv("metricas_mlp_z_variable.csv", index=False)
print("archivo 'metricas_mlp_z_variable.csv' generado.\n")
...
plt.title('Rendimiento Global: Real vs. Predicho (Log10)', fontsize=14, fontweight='bold')
...
plt.legend()
plt.tight_layout()
plt.savefig('diagnostico_1_real_vs_predicho.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.title('Residual Plot: Homocedasticidad y Patrones', fontsize=14, fontweight='bold')
plt.xlabel('Ganancia Predicha (Log10)', fontsize=12)
plt.ylabel('Residuo (Real - Predicho)', fontsize=12)
plt.tight_layout()
plt.savefig('diagnostico_2_residual_plot.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.title('Mapa Espacial del Error: Comportamiento Geométrico del MLP', fontsize=14, fontweight='bold')
plt.xlabel('Eje X: Longitud del Túnel (metros)', fontsize=12)
plt.ylabel('Eje Y: Ancho (metros)', fontsize=12)
plt.legend(loc='upper right')
plt.tight_layout()
plt.savefig('diagnostico_3_mapa_espacial_divergente.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.title('CDF: Probabilidad Acumulada del Error', fontsize=14, fontweight='bold')
plt.xlabel('Error Absoluto (Log10)', fontsize=12)
plt.ylabel('Proporción Acumulada', fontsize=12)
plt.legend(loc='lower right')
plt.tight_layout()
plt.savefig('diagnostico_4_cdf_percentiles.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.title('QQ-Plot: Evaluación de Normalidad de Residuos', fontsize=14, fontweight='bold')
plt.xlabel('Cuantiles Teóricos (Distribución Normal)', fontsize=12)
plt.ylabel('Cuantiles Ordenados de los Residuos', fontsize=12)
plt.tight_layout()
plt.savefig('diagnostico_5_qq_plot.png', dpi=300, bbox_inches='tight')
plt.close()
