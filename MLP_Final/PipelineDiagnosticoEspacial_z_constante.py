...
print("PIPELINE DE DIAGNOSTICO (Z CONSTANTE)")
...
X = df[["X", "Y", "Alpha", "Beta", "Distancia_3D", "Cos_Alpha", "Sin_Alpha", "Cos_Beta", "Inv_Dist_Sq", "Lambert_Factor"]]
...
# 2 CARGA DEL MEJOR MODELO Z CONSTANTE

print("cargando modelo pre-entrenado z constante")

mejor_mlp = joblib.load('modelo_mlp_z_constante_campeon_v2.joblib')
scaler = joblib.load('escalador_mlp_z_constante_campeon_v2.joblib')

X_test_scaled = scaler.transform(X_test)
y_pred = mejor_mlp.predict(X_test_scaled)
...
df_metricas.to_csv("metricas_mlp_z_constante.csv", index=False)
print("archivo 'metricas_mlp_z_constante.csv' generado\n")





# 4 GENERACION DE LOS GRAFICOS

print("generando los 5 graficos de diagnostico")
...
plt.title('Rendimiento Global: MLP Real vs. Predicho (Z Constante)', fontsize=14, fontweight='bold')
plt.xlabel('Ganancia Real (Simulada)', fontsize=12)
plt.ylabel('Ganancia Predicha (MLP)', fontsize=12)
r2_val = r2_score(y_test, y_pred)
...
plt.legend()
plt.tight_layout()
plt.savefig('diagnostico_const_1_real_vs_predicho.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.title('Residual Plot: MLP Z Constante', fontsize=14, fontweight='bold')
plt.xlabel('Ganancia Predicha (Log10)', fontsize=12)
plt.ylabel('Residuo (Real - Predicho)', fontsize=12)
plt.tight_layout()
plt.savefig('diagnostico_const_2_residual_plot.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.title('Mapa Espacial del Error: MLP Z Constante', fontsize=14, fontweight='bold')
plt.xlabel('Eje X: Longitud del Túnel (metros)', fontsize=12)
plt.ylabel('Eje Y: Ancho (metros)', fontsize=12)
plt.legend(loc='upper right')
plt.tight_layout()
plt.savefig('diagnostico_const_3_mapa_espacial.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.title('CDF: Probabilidad Acumulada del Error MLP Z Constante', fontsize=14, fontweight='bold')
plt.xlabel('Error Absoluto (Log10)', fontsize=12)
plt.ylabel('Proporción Acumulada', fontsize=12)
plt.legend(loc='lower right')
plt.tight_layout()
plt.savefig('diagnostico_const_4_cdf.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.title('QQ-Plot: Normalidad de Residuos MLP Z Constante', fontsize=14, fontweight='bold')
plt.xlabel('Cuantiles Teóricos', fontsize=12)
plt.ylabel('Cuantiles Ordenados de los Residuos', fontsize=12)
plt.tight_layout()
plt.savefig('diagnostico_const_5_qq_plot.png', dpi=300, bbox_inches='tight')
plt.close()
